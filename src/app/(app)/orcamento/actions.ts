...
/** Resultado das ações sem formulário: erro explícito, nunca falha em silêncio. */
 ...
 *
 * `upsert` em vez de read-then-write: sem a constraint do banco (migration 0013)
 * e com duas chamadas separadas, dois cliques simultâneos criavam dois
 * orçamentos para a mesma categoria — e o `maybeSingle()` seguinte passava a
 * quebrar com "more than one row".
 */
export async function saveBudget(categoryId: string, limitCents: number): Promise<ActionResult> {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  if (limitCents <= 0) {
    ...
    const { error } = await supabase.from("budgets").upsert(
      { user_id: user.id, category_id: categoryId, reference_month: null, limit_cents: limitCents },
